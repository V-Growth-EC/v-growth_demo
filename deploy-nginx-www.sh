...
# 上傳 nginx 設定檔
echo "📡 上傳 nginx-www3.conf..."
scp -i "$PEM_PATH" nginx-www3.conf $EC2_USER@$EC2_HOST:~/
...
ssh -i "$PEM_PATH" $EC2_USER@$EC2_HOST << EOF
  ...
  echo "📝 更新 nginx 設定檔..."
  sudo cp ~/nginx-www3.conf /etc/nginx/nginx.conf
...
echo "📋 檢查日誌: sudo tail -f /var/log/nginx/www3.edu-cart.jp.error.log" 